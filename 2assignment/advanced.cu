...
__global__ void computeHistogramOptimized(const unsigned char *imageIn, const int width, const int height, const int cpp) {
    ...
    // Reduce local histograms into global
    for (int i = tid; i < LUMINANCE_LEVELS; i += blockDim.x * blockDim.y)
...
int main(int argc, char *argv[])
{
    ...
    // Compute histogram optimized version
    computeHistogramOptimized<<<gridSize, blockSize>>>(d_imageIn, d_imageOut, width, height, cpp);
