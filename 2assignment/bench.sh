...
# List of images to benchmark
MODES="sequential basic advanced"
...
echo "Hostname,Image,Mode,TimeMemcpy1,TimeKernel1,TimeKernel2,TimeKernel3,TimeKernel4,TimeMemcpy2,TimeTotal" > "$OUTPUT_CSV"
...
for mode in $MODES; do
    ...
    for img in $IMAGES; do
        ...
        echo "Benchmarking $img with mode $mode"

        for run in $(seq 1 $repeat); do
            # Run the program and capture the output
            output=$("./$mode" "${IMGS_DIR}/$img" "out.png")
        ...
        echo "$img: Mode=$mode, Memcpy1=$avg_memcpy1, Kernel1=$avg_kernel1, Kernel2=$avg_kernel2, Kernel3=$avg_kernel3, Kernel4=$avg_kernel4, Memcpy2=$avg_memcpy2, Full=$avg_full"
        echo ""

        # Write to CSV
        echo "$(hostname),$img,$mode,$avg_memcpy1,$avg_kernel1,$avg_kernel2,$avg_kernel3,$avg_kernel4,$avg_memcpy2,$avg_full" >> "$OUTPUT_CSV"
    done
done
